...
#define DBG_FOR_EACH_RT_COL(M, r) \
    ...
    M(r, 16) M(r, 17) M(r, 18) M(r, 19) M(r, 20) M(r, 21) M(r, 22) M(r, 23) \
    M(r, 24) M(r, 25) M(r, 26) M(r, 27) M(r, 28) M(r, 29) M(r, 30)
...
#define DBG_SAMPLE_RT_CELL(r, c) \
    DBG_READ(rt_data[r][c], dbgIo_rt_table_##r##_##c);
...
void DebugManager::print_rob() {
    ...
    for (size_t i = 0; i < CFG_ROB_SIZE; ++i) {
        ...
        std::cout << std::format(
            ...
            e.prd_valid ? "Y" : "N",
            static_cast<uint8_t>(e.rename_ckpt_index));
        ...
        if (has_fc) {
            ...
            switch (static_cast<uint8_t>(dbg_u(fc.discon_type))) {
                ...
                default: discon_str = std::format("?{:#x}", static_cast<uint8_t>(dbg_u(fc.discon_type))); break;
            }
...
void DebugManager::print_rs() {
    ...
    for (size_t i = 0; i < CFG_RS_SIZE; ++i) {
        ...
        std::cout << std::format("{:<5x} {:<8} {:<10} {:#016x} {:<10} {:<10} {:#016x} {:#016x}\n",
                                ...
                                entry.params.source1,
                                entry.params.source2);
