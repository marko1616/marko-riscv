...
void VirtualAxiSlaves::handle_read(axiSignal &axi)
{
    ...
    case STAT_SEND:
        ...
        uint64_t current_addr =
            calculate_next_addr(current_read.addr, current_read.size, current_read.burst, current_read.beat, true);

        auto slave = std::ranges::find_if(slaves, [current_addr](const std::shared_ptr<Slave> &s) -> bool {
...
void VirtualAxiSlaves::handle_write(axiSignal &axi)
{
    ...
    case STAT_WRITE_DATA:
        axi.wready = true;
        if (axi.wvalid) {
            uint64_t current_addr = calculate_next_addr(current_write.addr, current_write.size, current_write.burst,
                                                        current_write.beat, false);

            auto slave = std::ranges::find_if(slaves, [current_addr](const auto &s) {
                return std::ranges::contains(s->range, current_addr - s->base_addr);
            });
