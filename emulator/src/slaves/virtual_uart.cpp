...
uint64_t VirtualUart::read(uint64_t addr, uint8_t size)
{
    ...
    case 0x2: { // IIR
        ...
        // THRE interrupt is cleared by reading IIR only if THRE is the reported
        // source.
    ...
    case 0x5: {         // LSR
        ...
        lsr_reg &= ~(LSR_DATA_OVERRUN | LSR_DATA_PARITY | LSR_DATA_FRAME | LSR_DATA_BREAK | LSR_FIFO_ERROR);
        iir_pending_reg &= ~PEND_LINE_STAT;
        update_iir();
        return ret;
    }
...
void VirtualUart::write(uint64_t addr, uint64_t data, uint8_t size, uint8_t strb)
{
    ...
    case 0x0: // THR / DLL
        ...
        if (mcr_reg & MCR_LOOPBACK) {
            ...
            // Write THR is complete immediately and atomically so we don't need to
            // change lsr;
    ...
    case 0x1: { // IER / DLM
        ...
        // If RX data is already waiting, enabling RX interrupt should expose it in
        // IIR.
        if ((ier_reg & IER_RX_AVAILABLE) && (lsr_reg & LSR_DATA_READY)) {
            iir_pending_reg |= PEND_RX_AVAIL;
        }

        // If THRE interrupt is newly enabled while THR is empty, raise one THRE
        // event.
        if (!(old_ier & IER_THR_EMPTY) && (ier_reg & IER_THR_EMPTY) && (lsr_reg & LSR_THR_EMPTY)) {
            iir_pending_reg |= PEND_THR_EMPTY;
        }
...
void VirtualUart::update_loopback_msr()
{
    ...
    if (changed & MSR_DCD)
        delta |= MSR_DDCD;
    if ((msr_reg & MSR_RI) && !(new_msr & MSR_RI))
        delta |= MSR_TERI;
