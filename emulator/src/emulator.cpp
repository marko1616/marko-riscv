...
void axi_debug(const axiSignal &axi)
{
    std::cout << std::format("AXI Signal State:\n"
                             ...
                             "  rdata:   0x{:016x}\n"
                             "  rresp:   0x{:02x}\n",
                             axi.awvalid, axi.awready, axi.awaddr, axi.awprot, axi.wvalid, axi.wready, axi.wdata,
                             axi.wstrb, axi.bvalid, axi.bready, axi.bresp, axi.arvalid, axi.arready, axi.araddr,
                             axi.arprot, axi.rvalid, axi.rready, axi.rdata, axi.rresp);
}

std::string cycle_verbose(uint64_t cycle, uint64_t pc, std::optional<uint32_t> raw_instr)
{
    std::string result =
        std::format("Cycle: 0x{:04x} PC: 0x{:016x} Instr: 0x{:08x} Asm: ", cycle, pc, raw_instr.value_or(0));
...
public:
    SimulationManager(parsedArgs &args) : args(args)
    {
        ...
        clint_id =
            slaves.register_slave(std::make_shared<VirtualCLINT>(0x02000000, args.timer_scale, args.stable_clock));
        ...
        uart_->set_interrupt_controller(std::dynamic_pointer_cast<VirtualPLIC>(slaves.get_slave(plic_id)));
    ...
    void run_simulation()
    {
        ...
        while (!Verilated::gotFinish() && clock_cnt < args.max_clock) {
            ...
            if (auto_pause_pending_.exchange(false, std::memory_order_relaxed)) {
                std::cerr << std::format("[AUTO-PAUSE] Event breakpoint hit at cycle=0x{:x} pc=0x{:x}\r\n", clock_cnt,
                                         debug_manager.curr_pc);
                input_manager->force_pause();
            }

            auto enqueue = [this](uint8_t ch) {
                uart_->enqueue_char(ch);
            };
            ...
            while (input_manager->is_paused()) {
                ...
                case InputManager::InputAction::PauseStep:
                    ...
                    execute_one_cycle(clock_cnt, debug_manager, axi);
                    clock_cnt++;
                    continue;
...
private:
    ...
    void execute_one_cycle(uint64_t clock_cnt, DebugManager &debug_manager, axiSignal &axi)
    {
        context->timeInc(1);
        top->clock = 1;
        top->eval();
    ...
    static bool filter_matches(const std::map<std::string, uint64_t> &filters, const std::string &key,
    ...
    static void load_payloads(const std::shared_ptr<VirtualRAM> &mem, const std::vector<PayloadSpec> &payloads,
                              const std::string &mem_name)
    {
        for (const auto &payload : payloads) {
            if (payload.type == PayloadType::Elf) {
                ...
                std::cout
                    << std::format("Loading {} BIN: {} @ {:#x}\n", mem_name, payload.file_path, payload.addr.value());
                mem->load_bin(payload.file_path, payload.addr.value());
            }
        }
    }

    void register_event_breakpoints(DebugManager &debug_manager)
    {
            ...
            if (bp.event_type == "issue") {
                ...
                debug_manager.on_discon([this, bp](const DisconEvent &e) {
                    if (filter_matches(bp.filters, "discon_type", e.discon_type) &&
                        ...
                        filter_matches(bp.filters, "prd", e.prd) && filter_matches(bp.filters, "prevprd", e.prevprd) &&
