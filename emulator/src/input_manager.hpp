...
private:
    ...
    // Process one byte. If in paused context, out_action receives the debug
    // command. Returns true if the byte was fully consumed (escape / debug
    // command).
    bool process_char(uint8_t ch, const CharCallback &enqueue_to_uart, bool paused_context, InputAction &out_action);

    void handle_escape(uint8_t ch, const CharCallback &enqueue_to_uart, bool paused_context, InputAction &out_action);
