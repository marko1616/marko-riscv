...
class CacheReadWriteWarpper(n_set: Int, n_way: Int, n_byte: Int)
    ...
    io.cache_invalid_addr.valid := false.B
    io.cache_invalid_addr.bits := 0.U
    ...
    when((io.invalid_addr.valid | io.write_req.valid | io.read_req.valid) && request_aligned) {
        ...
        for (i <- 0 until 2) {
            when(cache_line_addr(i) === (op_addr & cache_mask) && cache_line_buff(i).valid) {
                ...
                when(io.invalid_addr.valid) {
                    when(cache_line_buff(i).dirty) {
                        io.cache_write_req.valid := true.B
                        io.cache_write_req.bits.addr := cache_line_addr(i)
                        io.cache_write_req.bits.cache_line := cache_line_buff(i)

                        when(io.cache_write_outfire) {
                            cache_line_buff(i).dirty := false.B
                        }
                    }.otherwise {
                        io.cache_invalid_addr.valid := true.B
                        io.cache_invalid_addr.bits := cache_line_addr(i)
                        when(io.cache_invalid_outfire) {
