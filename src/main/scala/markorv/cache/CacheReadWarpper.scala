...
class CacheReadWarpper(n_set: Int, n_way: Int, n_byte: Int) extends Module {
    ...
    when(io.read_requests.valid && request_aligned) {
        ...
        for (i <- 0 until 2) {
            when(cache_line_addr(i) === (io.read_requests.bits.addr & size_mask) && cache_line_buff(i).valid) {
                // Cacheline hit.
                cache_line_hit := true.B
                io.read_data.valid := true.B
                raw_data := cache_line_buff(i).data >> (8.U*(io.read_requests.bits.addr & "h000000000000000f".U)(3,0))
            }
        }
