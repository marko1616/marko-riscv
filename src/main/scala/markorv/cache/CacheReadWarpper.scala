...
class CacheReadWarpper(n_set: Int, n_way: Int, n_byte: Int) extends Module {
    ...
    when(io.read_req.valid && request_aligned) {
        ...
        for (i <- 0 until 2) {
            when(cache_line_addr(i) === (io.read_req.bits.addr & cache_mask) && cache_line_buff(i).valid) {
                // Cacheline hit.
                cache_line_hit := true.B
                io.read_data.valid := true.B
                raw_data := cache_line_buff(i).data >> (8.U*(io.read_req.bits.addr(3,0) & size_mask(3,0)))
            }
        }
        ...
        when(!cache_line_hit) {
            ...
            when(io.read_cache_line.valid) {
                ...
                cache_line_addr(cache_replace) := (io.read_req.bits.addr & cache_mask)
                cache_replace := cache_replace + 1.U

                io.read_data.valid := true.B
                raw_data := io.read_cache_line.bits.data >> (8.U*(io.read_req.bits.addr(3,0) & size_mask(3,0)))
            }
        }
    }
