...
import markorv.config.{AxiConfig, CoreConfig, IOConfig}
...
class AXIHandler(val axiConfig: AxiConfig, val ioConfig: IOConfig, val id: Int)
    extends Module {
    ...
    class ReadState extends Bundle {
        val work = Bool()
        val bptr =
            if (burstLen != 0) Some(UInt(log2Ceil(burstLen + 1).W)) else None
    }
    ...
    if (ioConfig.read) {
        ...
        val rtemp =
            if (burstLen != 0) Some(RegInit(0.U(ioConfig.dataWidth.W)))
            else None
        ...
        when(~rstate.work && channel.params.valid) {
            ...
            io.axi.ar.bits.lock := (if (ioConfig.atomicity)
                                        channel.params.bits.lock.get
                                    else 0.U)
        ...
        when(rstate.work) {
            ...
            when(io.axi.r.valid) {
                ...
                when(io.axi.r.bits.last) {
                    ...
                    rstate.work            := false.B
                    if (burstLen != 0) {
                        channel.resp.bits.data := (io.axi.r.bits.data << (rstate.bptr.get << log2Ceil(
                          axiConfig.dataWidth
                        ))) | rtemp.get
                    } else {
                    ...
                    if (burstLen != 0) {
                        rtemp.get := (io.axi.r.bits.data << (rstate.bptr.get << log2Ceil(
                          axiConfig.dataWidth
                        ))) | rtemp.get
    ...
    if (ioConfig.write) {
        ...
        when(~wstate.work && channel.params.valid) {
            ...
            io.axi.aw.bits.lock := (if (ioConfig.atomicity)
                                        channel.params.bits.lock.get
                                    else 0.U)
        ...
        when(wstate.work && ~wstate.resp) {
            ...
            if (burstLen != 0) {
                val last =
                    Mux(wburst.get, wstate.bptr.get === burstLen.U, true.B)
...
class AxiRouter(val axiConfig: AxiConfig, val numChannel: Int) extends Module {
    ...
    val arArb = Module(
      new RRArbiter(new AxiReadAddressBundle(axiConfig), numChannel)
    )
    ...
    val awArb = Module(
      new RRArbiter(new AxiWriteAddressBundle(axiConfig), numChannel)
    )
    awArb.io.in.zip(io.axiChannel.map(_.aw)).zipWithIndex.foreach {
        case ((arbIn, chAw), i) =>
            arbIn <> chAw
            when(chAw.fire) {
    ...
    io.axiChannel.map(_.w).zipWithIndex.foreach { case (chW, i) =>
        chW.ready := writeChanOwner.valid && writeChanOwnerOH(
          i
        ) && io.axiBus.w.ready
    }
...
class AxiCtrl(implicit val c: CoreConfig) extends Module {
    val io = IO(new Bundle {
        val instrFetch = new IOInterface()(
          getCacheIoConfig(c.icacheConfig, CacheType.Icache),
          false
        )
        val dcacheLoadStore = new IOInterface()(
          getCacheIoConfig(c.dcacheConfig, CacheType.Dcache),
          false
        )
    ...
    val dcacheLoadStoreHandler = Module(
      new AXIHandler(
        c.axiConfig,
        getCacheIoConfig(c.dcacheConfig, CacheType.Dcache),
        1
    ...
    val dirLoadStoreHandler = Module(
      new AXIHandler(c.axiConfig, c.lsuIoConfig, 2)
    )
