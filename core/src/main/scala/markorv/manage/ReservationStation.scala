...
class ReservationStation(implicit val c: CoreConfig) extends Module {
    ...
    val dbgIo =
        if (c.simulate) Some(IO(Output(new ReservationStationDebugIO)))
        else None

    val regStates = io.regStates
    val buffer = RegInit(
      VecInit.tabulate(c.rsSize)(_ => new ReservationStationEntry().zero)
    )
    ...
    io.rsRegReqBits := buffer
        .map { entry =>
            val prs1Bit =
                Mux(entry.regReq.prs1Valid, 1.U << entry.regReq.prs1, 0.U)
            val prs2Bit =
                Mux(entry.regReq.prs2Valid, 1.U << entry.regReq.prs2, 0.U)
    ...
    finalParams.source1 := params.source1 + Mux(
      readyEntry.regReq.prs1Valid,
      regData1,
      0.U
    )
    finalParams.source2 := params.source2 + Mux(
      readyEntry.regReq.prs2Valid,
      regData2,
      0.U
    )
    ...
    // ========================
    when(io.flush) {
        buffer := VecInit.tabulate(c.rsSize)(_ =>
            new ReservationStationEntry().zero
        )
    }
