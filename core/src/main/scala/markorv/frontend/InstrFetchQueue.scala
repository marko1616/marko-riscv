...
class InstrFetchQueue(implicit val c: CoreConfig) extends Module {
    ...
    val lastHalfwordOffsetInLine =
        (c.icacheConfig.dataBytes - 2).U(lineOffsetBits.W)
    ...
    val fetchedInstrBits =
        (cachelineBits >> (cachelineByteOffset << 3.U))(31, 0)
    ...
    when(acceptFetchResp) {
        ...
        when(splitInstrPendingReg) {
            ...
            val isLow16PmaFault = splitInstrLow16Code.in(
              ICacheCode.pmaMmuWalkErr,
              ICacheCode.pmaInstErr
            )
            val isHigh16PmaFault = cachelineCode.in(
              ICacheCode.pmaMmuWalkErr,
              ICacheCode.pmaInstErr
            )
            ...
            val instrStatus = MuxCase(
              InstrStatus.instrOk32,
              Seq(
            ...
            assembledInstr.fromUInt(
              Cat(fetchedInstrBits(15, 0), splitInstrLow16Reg)
            )
            assembledInstr.status := instrStatus
        }.otherwise {
            val isPmaFault = cachelineCode.in(
              ICacheCode.pmaMmuWalkErr,
              ICacheCode.pmaInstErr
            )
