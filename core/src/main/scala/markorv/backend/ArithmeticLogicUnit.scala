...
class ArithmeticLogicUnit(implicit val c: CoreConfig) extends Module {
    ...
    val sllShift = Mux(op32, source1 << source2(4, 0), source1 << source2(5, 0))

    val srlShift =
        Mux(op32, source1(31, 0) >> source2(4, 0), source1 >> source2(5, 0))
