...
import markorv.bus.{IOInterface, ReadParams, WriteParams}
...
class LoadStoreUnit(implicit val c: CoreConfig) extends Module {
    ...
    object State extends ChiselEnum {
        val sIdle, sCacheReadWait, sCacheWriteWait, sBypassRead, sBypassWrite,
            sAmoFlushWait, sAmoRead, sAmoWrite = Value
    }
    ...
    val cacheWriteMaskBase = MuxLookup(size, 0.U(8.W))(
      Seq(
        0.U -> "h01".U(8.W),
        1.U -> "h03".U(8.W),
        2.U -> "h0f".U(8.W),
    ...
    val source2Extended = MuxLookup(size, params.source2.asUInt)(
      Seq(
    ...
    val amoAluResult = MuxLookup(opcode, 0.U)(
      Seq(
        ...
        LSUOpcode.amomin -> Mux(
          amoDataReg.asSInt < source2Extended.asSInt,
          amoDataReg,
          source2Extended
        ),
        LSUOpcode.amomax -> Mux(
          amoDataReg.asSInt > source2Extended.asSInt,
          amoDataReg,
          source2Extended
        ),
        LSUOpcode.amominu -> Mux(
          amoDataReg < source2Extended,
          amoDataReg,
          source2Extended
        ),
        LSUOpcode.amomaxu -> Mux(
          amoDataReg > source2Extended,
          amoDataReg,
          source2Extended
        )
        ...
        is(State.sAmoFlushWait) {
            when(io.cacheAmoFlushResp.valid) {
                ...
                when(code.isOk()) {
                    ...
                    when(isSc) {
                        when(
                          localLoadReservedValid && localLoadReservedPa === io.paddr
                        ) {
