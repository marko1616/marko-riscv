...
class DataCache(implicit val c: CacheConfig) extends Module {
    ...
    val reqWriteSet  = if(c.setNum == 1) 0.U else reqWriteVa(c.setEnd, c.setStart)

    val reqCleanVa = io.cacheInterface.cleanReq.bits.vaddr
    val reqCleanSet  = if(c.setNum == 1) 0.U else reqCleanVa(c.setEnd, c.setStart)

    val reqInvalidateVa = io.cacheInterface.invalidateReq.bits.vaddr
    val reqInvalidateSet  = if(c.setNum == 1) 0.U else reqInvalidateVa(c.setEnd, c.setStart)

    val reqAmoFlushVa = io.cacheInterface.amoFlushReq.bits.vaddr
    val reqAmoFlushSet  = if(c.setNum == 1) 0.U else reqAmoFlushVa(c.setEnd, c.setStart)
    ...
    val reqPaReadSet = if(c.setNum == 1) 0.U else reqPaReadPa(c.setEnd, c.setStart)
    ...
    io.cacheInterface.paddr := Mux(state === State.sRead, Cat(mmuPaHigh, transactionPaLow), transactionPa)
    ...
    val mmuPrivValid = MuxLookup(loadStorPriv, false.B)(Seq(
        "b00".U -> mmuPrivValidU,
        "b01".U -> mmuPrivValidS,
        "b11".U -> mmuPrivValidM
    )) || !useProt
    ...
    val readPteValid = io.mmuResp.bits.accessed && (io.mmuResp.bits.pteRead || (io.statusMxrField && io.mmuResp.bits.pteExec))
    val writePteValid = io.mmuResp.bits.accessed && io.mmuResp.bits.dirty && io.mmuResp.bits.pteWrite
    ...
    val amoFlushTransactionPteValid = Mux(amoFlushReadLike, readPteValid, writePteValid)
    ...
    val amoFlushTransactionValid = io.mmuResp.bits.pmaWrite && io.mmuResp.bits.atomic
    ...
    val transactionShiftedWriteData = shiftWriteData(transactionWriteData, transactionByteOffset)
    val transactionShiftedWriteMask = shiftWriteMask(transactionWriteMask, transactionByteOffset)
    val mergedWriteData = mergeWriteData(transactionReadData, transactionShiftedWriteData, transactionShiftedWriteMask)
    ...
    val paReadSafe = (state === State.sIdle) || ((state === State.sRead) && !io.mmuResp.valid)
    ...
    val readReqReadyNoMmu = canAcceptLocalReq && !io.cacheInterface.paReadReq.valid
    val writeReqReadyNoMmu = canAcceptLocalReq && !io.cacheInterface.paReadReq.valid && !io.cacheInterface.readReq.valid
    ...
    switch(state) {
        is(State.sIdle) {
            ...
            val reqWithAddrNextState = Mux(transactionReqVaFired, State.sRead, State.sIdle)
            val reqForAllNextState = MuxCase(State.sIdle, Seq(
                startInvalidateAll -> State.sInvalidateAll,
                startCleanAll -> State.sCleanAll
            ))
            val finalNextState = Mux(transactionReqVaFired, reqWithAddrNextState, reqForAllNextState)
        ...
        is(State.sRead) {
            ...
            when(io.mmuResp.valid) {
                ...
                val pmaFault = MuxLookup(transactionType, false.B)(Seq(
                ...
                val pteFault = MuxLookup(transactionType, false.B)(Seq(
                ...
                val transactionFault = mmuWalkPmaFault || !mmuCommonValid || pmaFault || pteFault || !transactionPmaCacheable

                val pmaFaultCode = MuxLookup(transactionType, DCacheCode.pmaLoadErr)(Seq(
                    ...
                    TransactionType.amoFlush   -> Mux(amoFlushReadLike, DCacheCode.pmaLoadErr, DCacheCode.pmaStorErr)
                ))
                val pteFaultCode = MuxLookup(transactionType, DCacheCode.pageLoadErr)(Seq(
                    ...
                    TransactionType.amoFlush   -> Mux(amoFlushReadLike, DCacheCode.pageLoadErr, DCacheCode.pageStorErr)
                ))
                // First match wins in MuxCase -> walk > pma > pte
                val faultCode = MuxCase(DCacheCode.pmaMmuWalkErr, Seq(
                ...
                val respData = Mux(!transactionFault && readHit && isRead, shiftLineData(hitData, transactionByteOffset)(63, 0), 0.U)
                ...
                when(!transactionFault) {
                    ...
                    val needWorkSet = (readHit && (isWrite || ((isClean || isAmoFlush) && hitDirty))) ||
                    ...
                    when(readHit && (isInvalidate || (isAmoFlush && !hitDirty))) {
                ...
                normalNextState := MuxCase(State.sRead, Seq(
                    ...
                    (!transactionFault && !readHit && (isRead || isWrite)) -> Mux(victimDirty, State.sVictimWriteBack, State.sReplace)
                ))
            }
            ...
            val pipelineNextState = Mux(transactionReqVaFired, reqWithAddrNextState, reqForAllNextState)
        ...
        is(State.sReplace) {
            ...
            when(io.ioInterface.read.get.resp.valid) {
                ...
                val refillRespData = shiftLineData(io.ioInterface.read.get.resp.bits.data, transactionByteOffset)(63, 0)
                ...
                when(transactionType === TransactionType.read) {
                    ...
                    when(refillOk) {
                        ...
                        io.cacheInterface.writeResp.valid := true.B
                        io.cacheInterface.writeResp.bits.code.fromAxiResp(io.ioInterface.read.get.resp.bits.resp, false.B)
                        state := State.sIdle
                    }
                }
            }
        ...
        is(State.sVictimWriteBack) {
            val dirtyVictimPa = if (c.setNum == 1) Cat(workSetTagVReg(victimPtr).tag, 0.U(c.offsetBits.W)) else Cat(workSetTagVReg(victimPtr).tag, transactionSet, 0.U(c.offsetBits.W))

            io.ioInterface.write.get.params.valid := true.B
            io.ioInterface.write.get.params.bits.addr := dirtyVictimPa
            io.ioInterface.write.get.params.bits.data := workSetDataReg(victimPtr).data
        ...
        is(State.sCleanWriteBack) {
            ...
            when(io.ioInterface.write.get.resp.valid) {
                ...
                when(cleanWriteBackIsCleanAll) {
                    ...
                    when(transactionType === TransactionType.amoFlush && writeBackOk) {
        ...
        is(State.sCleanAll) {
            when(!cleanAllSetLoaded) {
                ...
                when(cleanAllSetTagVReg(cleanAllWayPtr).valid && cleanAllSetDirtyReg(cleanAllWayPtr).dirty) {
    ...
    paReadHitData := Mux(paReadHit, Mux1H(paReadHits, paReadSramReadData.map(_.data)), 0.U)
    ...
    switch(paReadState) {
        ...
        is(PaReadState.sLookup) {
            when(paReadHit) {
                ...
                io.cacheInterface.paReadResp.bits.data := shiftLineData(paReadHitData, paReadByteOffset)(63, 0)
        ...
        is(PaReadState.sMiss) {
            ...
            when(io.ioInterface.read.get.resp.valid) {
                io.cacheInterface.paReadResp.valid := true.B
                io.cacheInterface.paReadResp.bits.code.fromAxiResp(io.ioInterface.read.get.resp.bits.resp, false.B)
                io.cacheInterface.paReadResp.bits.data := shiftLineData(io.ioInterface.read.get.resp.bits.data, paReadByteOffset)(63, 0)
                paReadState := PaReadState.sIdle
            }
        }
    }
    ...
    when(finalSetTransactionValid) {
        ...
        transactionAddrLow :=
            Mux(sIdleSetTransactionValid, sIdleSettransactionAddrLow, sReadSettransactionAddrLow)
    ...
    when(finalSetTransactionPaHighValid) {
        transactionPaHigh :=
            Mux(sIdleSetTransactionPaHighValid, sIdleSetTransactionPaHigh, sReadSetTransactionPaHigh)
    }
    ...
    when(finalSetCleanWriteBackValid) {
        ...
        cleanWriteBackPa :=
            Mux(sReadSetCleanWriteBackValid, sReadSetCleanWriteBackPa, sCleanAllSetCleanWriteBackPa)
        cleanWriteBackData :=
    ...
    sramReadDirty := dirtyArray.read(finalMainSramReadSet, finalMainSramReadValid)
    ...
    paReadSramReadDirty := dirtyArray.read(paReadReadSramSet, paReadReadSramValid)
}